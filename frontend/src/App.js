...
export default function App() {
  ...
  // Shared button styles for Payments tab toolbar
  const toolbarBtn = (active, color = "#3a3f5a") => ({
    background: active ? color : "#12141e",
    color: active ? "#0b0d14" : "#3a3f5a",
    border: `1px solid ${active ? color : "#191d2e"}`,
    borderRadius: 8, padding: "7px 16px", cursor: "pointer",
    ...S.mono, fontSize: 11, letterSpacing: 1, transition: "all .15s",
  });
    ...
    <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", background: "#0b0d14", minHeight: "100vh", color: "#ddd8cc" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["all","All"],["income","Income"],["expense","Expenses"]].map(([v,l]) => (
                    <button key={v} onClick={() => setFilterType(v)} style={toolbarBtn(filterType === v, "#c4a24a")}>{l}</button>
                  ))}
                </div>

                {/* Action buttons */}
                <div style={{ display: "flex", gap: 8 }}>
                  {/* Export — only useful when there's data */}
                  {entries.length > 0 && (
                    <button
                      ...
                      style={{
                        background: "#12141e",
                        color: exportLoading ? "#3a3f5a" : "#7a8299",
                        border: "1px solid #191d2e",
                        borderRadius: 8, padding: "7px 16px", cursor: exportLoading ? "wait" : "pointer",
                        ...S.mono, fontSize: 11, letterSpacing: 1, transition: "color .15s",
                      }}
                    >
                  ...
                  <button
                    onClick={openAdd}
                    style={{ background: "#c4a24a", color: "#0b0d14", border: "none", borderRadius: 9, padding: "9px 20px", ...S.mono, fontSize: 11, cursor: "pointer", letterSpacing: 1, fontWeight: 600 }}
                  >
                    + Add Entry
                  </button>
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {listEntries.length === 0 && (
                  <div style={{ color: "#3a3f5a", textAlign: "center", padding: 40, ...S.mono, fontSize: 11 }}>
      ...
      {showImportModal && (
        <ImportModal
          ...
          onImported={() => {
            loadEntries(); // refresh the entry list after a successful import
          }}
        />
      )}
    </div>
